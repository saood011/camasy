...
export const getSingleStudentDetails = id => dispatch => {
  dispatch(enableStudentDetailsLoading());
  axios
...
/* export const handleOnDateClick = info => dispatch => {
  console.log("handle Clicked" + info);
  axios
    .post(`/api/student/attend/addAttendance`, info)
    .then(res => {
      dispatch({
        type: GET_STUDENT_DETAILS,
        payload: res.data
      });
      console.log("hande click from studentAction" + res);
    })
    .catch(err => {
      console.log(err);
      dispatch({
